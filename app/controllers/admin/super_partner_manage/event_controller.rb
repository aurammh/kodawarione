class Admin::SuperPartnerManage::EventController < ApplicationController
  before_action :authenticate_admin!
  before_action :set_super_partner_event, only: %i[event_details event_edit event_update event_delete]
  ...
  def index
    ...
    if startDate.blank? && !endDate.blank?
        @partner_events = Event.super_partner_event_search_by_only_end_date(date_type, endDate, keyword, [event_category_data])
    elsif(!startDate.blank? && endDate.blank?)
        @partner_events = Event.super_partner_event_search_by_only_start_date(date_type, startDate, keyword, [event_category_data])
    elsif(!startDate.blank? && !endDate.blank?)
        @partner_events = Event.super_partner_event_search_by_between_two_date(date_type, startDate, endDate, keyword, [event_category_data])   
    else
        @partner_events = Event.super_partner_event_search_all_list(keyword, [event_category_data])
    end        
    @partner_events = Kaminari.paginate_array(@partner_events).page(params[:page]).per(12)
  end
  ...
  def event_update
    respond_to do |format|
      if @partner_event.update(event_params)
        ...
        format.html { redirect_to admin_super_partner_manage_event_details_path(:id => @partner_event.id), notice: "Event was successfully updated." }
  ...
  def event_delete
    ...
    redirect_to admin_super_partner_manage_event_search_path
  end
  
  private
  def set_super_partner_event
